    ...
    fs.flock(fd, "ex", function (err) {
      ...
      fs.fstat(fd, function (err, stat) {
        ...
        handle_read = function (err, bytes_read, buf) {
          if (err) {
            log.error("Error reading", _path, ":", err);
            return cb(err);
          }
          total_bytes_read += bytes_read;
          if (total_bytes_read < stat.size) {
            log.debug(util.format("%s bytes left to read", stat.size - total_bytes_read));
            return fs.read(fd, buf, total_bytes_read, stat.size - total_bytes_read, total_bytes_read, handle_read);
          }
          fs.close(fd, function (err) {
            ...
            try {
              log.debug(_path, "data:", buf.toString());
